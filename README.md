...
api/
  ...
  └─ docs.js       ← función serverless (lee el repertorio desde tu Drive)
README.md          ← esto
```
...
| `GOOGLE_API_KEY`     | *(opcional)* clave de Google para auto-importar el repertorio desde Drive |

Los nombres deben ser **exactos** (mayúsculas y guiones bajos igual que arriba).

## Repertorio automático desde Drive (opcional)

La herramienta ya trae todo el repertorio incrustado (dossiers, presupuestos y
recursos). Si quieres que se **actualice solo** cada vez que subes un archivo a
Drive (sin tocar código), activa `/api/docs`:

1. **Comparte la carpeta maestra de Drive** como *"Cualquiera con el enlace ·
   Lector"* (los recursos deben ser públicos para que el lead pueda abrirlos de
   todos modos).
2. **Crea una API key de Google (gratis):** console.cloud.google.com → crea un
   proyecto → *APIs y servicios* → habilita **"Google Drive API"** → *Credenciales*
   → **Crear credenciales → Clave de API**. Copia la clave.
3. En Vercel añade la variable `GOOGLE_API_KEY` con esa clave (y opcionalmente
   `DRIVE_FOLDER_ID` si cambias de carpeta; por defecto usa la carpeta de recursos).
4. Redeploy. La web leerá tu Drive sola (máx. 1 vez cada 6 h) y con el botón
   **«🔄 Actualizar desde Drive»** del repertorio. Aplica **Explora > Ucademy**
   automáticamente y respeta la estructura de carpetas (Dossiers / Presupuestos /
   One Pagers / Otros). Si la key no está, la web sigue funcionando con el
   repertorio incrustado.

> Convención de nombres que la web entiende: las carpetas `💜 …` = Explora y
> `💚 UCADEMY` = Ucademy; en presupuestos usa la versión normal (ignora las " MG",
> "Febrero" y "enero"); el nombre del archivo debe incluir la formación.
